...
> Отслеживание изменений с помощью хеш-сумм; алгоритм SHA-1 (40 шестнадцатеричных символов).
...
`git clone <ssh/https link>` - clone remote repo.
  - `--branch <branch>  --single-branch` - клонировать одну конкретную ветку
...
`git reflog` - подродный лог репозитория с описанием операций, со ссылками (HEAD~), которые можно использовать  
чтобы переключиться на любое состояние.
...
`git checkout .` - отмена всех не индексированных локальных изменений.
...
`git switch <branch>` - переключиться на ветку; более подходящая команда для этого, чем git checkout.
  - `-c/--create` - создать ветку от текущей и переключиться на неё
...
`git push` - загружает изменения из локального репозитория на удалённый.
  ...
  - `--force` - принудительное влитие локальной ветки в удалённую; например при локальной отмене коммита (без этого флага попросит сначала спулить)
...
- `recursive` - (устарела) рекурсивная; работает с двумя указателями HEAD; default для merge/pull одной ветки.
- `ort` - (ostensibly recursive's twin); дефолтная стратегии вместо рекурсивной; быстрее рекурсивной.
...
можно перенести последовательность коммитов `git cherry-pick <hash_from>..<hash_to>` (работате не очень - с конфликтами,  
лучше объединить несколько коммитов через `git rebase -i HEAD~n` и один коммит уже черрипикнуть).
- `--abort` - отмена
___
