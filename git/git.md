...
`.gitkeep` - пустой файл в корне проекта, позволяет сохранять пустые директории при коммите.
...
`git rebase <base>` - переместить текущую ветку на последний коммит ветки base.
