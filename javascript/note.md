...
`lodash.noop()` - (no operation) возвращает undefined независимо от переданных аргументов; используется как  
значение по умолчанию.
___

## confirm

`confirm(question)` - часть браузерного апи; выводит модалку с вопросом; возвращает boolean, в зависимости от  
ответа.
...
`!function() {}()` - по сути тоже что `(function() {})()`, но экономит байт на скобочку наверно; IIFE обычно ничего не  
возвращает, т.е. undefined, но при вызове через ! выражение вернёт true (!undefined == true).
