...
> Ленивые/отложенные вычисления - стратегия вычисления, когда оно откладывается пока не понадобится результат.  
> Есть во многих ЯП (особенно haskell).

```
// немедленное вычисление при присваивании
const res = 1 + 2;

// отложенное вычисление, выполнится при обращении
const res = () => 1 + 2;
```

```
// ленивые вычисления в логических выражениях js

true || foo1;  // foo1 не выполнится пока слева true

false && foo2;  // foo2 не выполнится пока слева false
```

**Ленивые коллекции** - позволяют работать с бесконечными списками за счёт ленивого вычисления (в js есть  
библиотеки, в некоторых языках из коробки).

```
Lazy.generate(Math.random)
  .map(...)
  .uniq(...)
  .take(300);  // вычисления до take не выполнятся, пока take не сработает, иначе было бы переполнение стека
```

```
// класс с ленивым вычислением
...
class MyClass {
  ...
  select(fn) {
    const newOps = this.operations.slice();   // slice() без аргументов копирует массив

    newOps.push(coll => coll.map(fn));

    return new MyClass(this.coll.slice(), newOps);
  }

  toArray() {
    return this.operations.reduce((acc, fn) => fn(acc), this.coll);
  }
}
```
...
**getter** - динамическое свойство класса; снаружи выглядит как свойство, внутри вычисляется вызовом  
get-функции.

```
class MyClass {
  get length() {...}  // getter не может принимать аргументы
}
...
**Мемоизация** - паттерн сохранение результата выполнения функции для предотвращения повторных вычислений;  
мемоизировать надо чистые функции.

```
// при первом вызове сохранит результат в memo, при повторных возвращает memo без вычисления
...
**Rest operator** - оператор остатка, используется только в конце аргументов функции.
...
fn('first');  // first undefined []

// [] - дефолтное значение пустого rest-оператора
```

**Spread** - операция обратная rest - растягивает коллекцию на отдельные элементы; в отличии от rest может стоять везде.
...
**Immutable way** - создавать новые структуры используя spread.

```
[...arr, 3, 4]
{...obj, two: 2}  // если obj имеет свойство two, то запишется правое значение
```

```
// immutable reduce (по сути здесь reverse)
// на больших данных spread сильно затратнеe push

[].reduce((acc, cur) => [cur, ...acc], []);

// дублирование значений

arr.reduce((acc, cur) => [...acc, cue, cur], []);
