...
`??` - nullish coalescing оператор; проверка на null, undefined; возвращает значение правого операнда, когда  
значение левого равно null, undefined; в противном случае вернёт значение левого операнда.
...
`break` - прерывает цикл принудительно.

`continue` - прерывает текущую итерацию, но не цикл; итерация пропускается.
___
...
**Недостижимые** - объекты на которые никто не ссылается и они нигде не используются; может иметь слабые ссылки  
на себя.

**Слабая ссылка** - позволяет работать с объектом (как сильная ссылка), но не учитывается сборщиком мусора, поэтому  
при необходимости объект может быть удалён; например MutationObserver использует слабые ссылки на наблюдаемые ноды.
...
**Экспоненциальная форма числа** - часть числа заменяется на `e+n`, где `e` - экспонента, умножает число слева на  
10 в степени `n`.

```
6.8e+2 === 6.8 * 10 ** 2;  // true
6.8e+2 === 680;  // true
```
...
- `num.toFixed(n)` - округление до указанного знака после запятой; без аргумента делает математическое округление;  
  делает математическое округление; возвращает string.  
- `num.toPrecision(n)` - n - количество знаков всего; округляет математически; вернёт строку.
...
`Math.abs(n)` - вернёт модуль числа (т.е. отрицательное сделает положительным).

`Math.min([...numbers])` - вернёт минимальное число из переданных; без аргументов вернёт Infinity.
...
`Math.max([...numbers])` - вернёт максимальное число.

`Math.sqrt(n)` - вернёт квадратный корень числа (только положительное число).

`Math.pow(n, exponent)` - возводит n в степень exponent; степень может быть отрицательной (`a ** -n = 1 / a ** n`);  
...
`Math.random()` - вернёт случайное дробное число от 0 до 1.
...
`Infinity` `-Infinity` `NaN` - формально относятся к типу number.

`isNaN` - проверка на NaN, т.к. NaN не равен ни чему, даже самому себе.

`isFinite()` - проверяет является ли число конечным; вернёт true для любого числа, но false - для NaN, Infinity,  
-Infinity.

```
NaN === NaN;  // false
isNaN(NaN);  // true
Number.isNaN(NaN);  // то же
...
`parseInt(string, [base])` - парсит целое число из строки типа `'10px'`; base - опциональное  
основание системы счисления (2-36), по дефолту 10; можно использовать для округления до целого.

`parseFloat(string, [base])` - то же но для числа с запятой (не округляет).
...
`.padStart(length, fillString)` - заполняет строку fillString'ами так, чтобы итоговая строка была длины length.

`.padEnd(length, fillString)` - то же, но заполняет с конца строки.
```
'9'.padStart(4, '0');  // '0009'
'9'.padEnd(4, 0);  // '9000'
```
...
`.toLowerCase()`  
`.toUpperCase()` - преобразует строку к нижнему/верхнему регистру; возвращает новую строку, не меняет исходную.

`.lastIndexOf()`
`.indexOf(substring, from)` - ищет подстроку в строке; вернёт позицию последнего/первого совпадения или -1;  
from - откуда начать поиск.

`.replace()`
`.replaceAll(str|regexp, str|cb)` - поиск и замена первого совпадения/всех совпадений подстроки в строке; не мутирует  
исходную строку.

`.split(splitter, maxLength)` - разбивает строку в массив по указанному разделителю; не изменяет исходную строку.
___
...
`.push()`  
`.unshift()` - добавить элемент (или несколько через запятую) в конец/начало массива; мутируют исходный массив;  
вернёт новую длину массива.

`.pop()`  
`.shift()` - удалить последний/первый элемент массива; возвращает сам удалённый элемент.
___
...
`.slice(from, to)` - возвращает указанную часть массива; не изменяет исходный массив; без аргументов делает копию  
массива.
___

`.splice(from, count, [...items])` - изменяет содержимое массива, добавляет/удаляет элементы; items - добавляемые  
элементы; изменяет исходный массив; возвращает изменённый массив.

```
arr.splice(2, 0, 'one', 'two');  // вставит 2 элемента перед вторым элементом.

arr.splice(2, 3, 'one', 'two');  // удалить 3 элемента, начиная со второго; вставит 2 элемента перед вторым элементом.
```
___
...
`.sort(cb)` - без функции сравнения в аргументе сортирует в лексиграфическом порядке (т.е. элементы приводятся к  
строке и сравниваются как строки); мутирует исходный массив.

Для функции сравнения достаточно вернуть любое положительное/отрицательное число.

```
[2, 1].sort((a, b) => a - b);  // [1, 2]. для обратного порядка b - a

// сортирует в случайном порядке
arr.sort(() => 0.5 - Math.random())  // функция возвращает <0, 0, >0
...
`.some(fn)`  
`.every(fn)` - если callback-функция для какого-либо/всех элементов вернёт true, то метод вернёт true, иначе false.
...
`.fill(value, start, end)` - заполнит массив одинаковыми элементами от start до end (не включая end);  
...
`.copyWithin(target, start, end)` - копирует элементы между start и end и вставляет начиная с индекса target.
...
`.reverse()` - меняет порядок элементов массива на обратный; мутирует исходный массив.
___

`.filter(cb)` - создаёт новый массив; отбрасывает элементы не прошедшие условие в callback; не изменяет исходный  
массив.

```
wordsArr.filter((word) => word.length > 6);
```
___
___
## Iterable objects
...
`.toLocaleDateString(locale, opt)`  
`.toLocaleTimeString(locale, opt)`  
`.toLocaleString(locale, options)` - форматирует дату/время/строке в местном представлении; также есть у всех встроенных  
объектов (Number, String, etc); по сути укороченный вариант, без вызова `new Intl`.

```
10..toLocaleString('ru', {style: 'currency', currency: 'RUB'});  // '10,00 ₽'

new Date().toLocaleString('ru', {year: 'numeric', month: 'long'});  // 'октябрь 2023 г.'

new Date().toLocaleDateString('ru');  // '17.10.2023' - представление по дефолту

new Date().toLocaleTimeString('ru');  // '13:48:00'
```
___
...
дальше по цепочке; не знает как разрезолвился промисы не имеет доступа к результату/ошибке.
...
`Promise.all([...promises])` - вернёт промис, который выполнится когда завершатся все переданные (обычно в массиве) промисы; если хоть один из переданных промисов завершится с ошибкой, то результирующий промис завершится с ошибкой,  
игнорируя успешные промисы; выполняет промисы параллельно.
...
Производят операции, используя двоичное представление числа (последовательность из 32 бит).
___

**Знаковые бит** -крайний левый бит; если `0`, то число положительное, если `1` то отрицательное.
___

Сделать число **отрицательным**: 
1. обратить все биты (`0` в `1`, `1` в `0`)
2. прибавить `1` двоичным сложением
___
...
`n.toString(2)` - двоичное представление n в формате строки.

`parseInt('101', 2)` - переводит в десятичное число.
___

`&` - (побитовое И) выполняет И над каждой парой бит; вернёт `1` только если оба оператора == 1.
...
`|` - выполняет ИЛИ над каждой парой бит; вернёт 1, если хоть один оператор == 1.
...
`^` - исключающее ИЛИ, вернёт 1 если либо слева 1, либо справа 1, но не оба == 1.

Используется для **шифрования** или как часть шифрующего алгоритма.
...
Т.к. `^` это обратимая операция, то её двойное применение с теми же аргументами вернёт исходное число.

`a ^ b ^ b == a`, `b` - ключ для шифрования/дешифрования.
___

`~` - побитовое НЕ; меняет каждый бит на обратный.
...
В десятичной системе будет `~n == -(n + 1)`.
___

`<<` - побитовый сдвиг влево; сдвигает число на указанное количество бит влево, добавляя нулевые биты справа; лишние  
биты слева отбрасываются.

```
9 << 2  // 36, т.к. 1001 -> 100100 (== 36)

// по сути то же что умножение 9 * 2 два раза
```
___

`>>` - правый битовый сдвиг; сдвигает вправо, отбрасывая лишнее; добавляет копию знакового бита столько раз, на сколько  
сдвинуто число.

```
9 >> 2  // 2,  1001 -> 10 (== 2)

// то же что деление на 2 два раза
```
___

`>>>` - правый сдвиг с заполнением нулями.

Для положительного числа то же что `>>`, для отрицательного результат будет всегда положительный, т.к. знаковый бит  
заменится на 0 при сдвиге.
___

У побитовых операторов **низкий приоритет**, ниже чем `==`.
___

**Битовая маска** - комбинация двоичных значений, которая используется для проверки наличия единиц на нужных позициях.

Можно использовать для передачи большого количества булевых флагов (до 32) в компактном виде.
___

Побитовые операторы работают с числами длиной **32 бита**; обычные математические операторы JS работают с числами длиной  
64 бита.
___
...
// проверка на -1 (например indexOf вернёт -1, если не нашел элемент)
if (~str.indexOf(subStr)) {
  alert('catch!'); 
}

// умножение/деление на 2 в степени n
3<<3  // 24
8>>2  // 2 (8 делить на 2 два раза)
11>>2  // 2 дробная часть отброшена
...
`Intl` - встроенный объект с методами для интернационализации.

**Основные объекты:**
- `Intl.Collator()` - для сравнения, сортировки строк.
- `Intl.DateTimeFormat()` - форматирует дату локальный стандарт.
- `Intl.NumberFormat()` - форматирует числа.
___

`locale` - первый аргумент во всех методах Intl, передается в виде строки.

**Состоит из** трёх компонентов, разделённых через дефис:
1. код языка (минимально необходимое)
2. код способа записи
3. код страны

`th-TH-u-nu-thai` - также через `-u` можно указать расширение локали.

Если локаль не указана, то берётся из дефолтных настроек браузера.

**Код языка:**

`ru` - русский, без уточнений

`en-US` - английский США

`zn-Hans-CN` - китайский упрощённый, используемый в Китае.
___

### Intl.Collator

`Intl.Collator(locale, options)` - вернёт объект с методом `compare`
- `locale` - строка/массив строк
- `options` - опциональный объект настроек
  - `localeMatcher` - алгоритм выбора локали: 
    - `'lookup'`  
    - `'best fit'` - default
  - `usage` - цель сравнения:
    - `sort` - default
    - `search`
  - `sensitivity` - чувствительность к различиям:  
    - `base` - a != b, е == ё, a == A
    - `accent` - a != b, е != ё, a == A
    - `case` - a != b, е == ё, a != A
    - `variant` - всё !=, дефолтный
  - `ignorePunctuation` - false (default)
  - `numeric` - если true, то `1 < 2 != 1 > 2`
  - `caseFirst` - идут первые при сортировке:  
    - `upper`
    - `lower`
    - `false` - default
...
const result = collator.compare('днем', 'днём');
...
`Intl.DateTimeFormat(locale, options)` - вернёт объект с методом `format`.
- `options`:
  - `localeMatcher`:
    - `lookup`
    - `best fit`
  ...
  - `timeZone` - например: 'Europe/Moscow'; UTC - default
  - `weekday` - narrow, short, long
  - `era` - narrow, short, long
  - `year` - 2-digit, numeric (default)
...
formatter2.format(new Date);  // '12:30:00'
```
___

### Intl.NumberFormat

`Intl.NumberFormat(locale, options)` - форматер чисел, валют, процентов; вернёт объект с методом `format`.
- `options`:
  - `localeMatcher` - lookup, best fit (default)
  - `style` - decimal (def), percent, currency
  - `currency` - код валюты, например USD
  - `currencyDisplay` - code, symbol (def), name
  - `useGrouping` - делить цифры на группы; true (def), false
  - `minimumIntegerDigits` - 1-21 (21 - default)
  - `minimumFractionDigits` - количество десятичных цифр; 0-20
  - `minimumSignificantDigits` - 1-21 (1 - default); по сути округление
  - `maximumSignificantDigits` - minimumSignificantDigits-21; тоже про округление

```
new Intl.NumberFormat('ru').format(1234.12);  // '1234,12'
