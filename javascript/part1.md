...
`str.padStart(length, fillString)` - заполняет строку fillString'ами так, чтобы итоговая строка была длины length.

`str.padEnd(length, fillString)` - то же, но заполняет с конца строки.
