...
- Path-параметры встраиваются в сам url. Обычно используется для уточнения специфики ресурса, идентификации.
`https://example.com/api/user/123` - '123' - path-параметр

- Query-параметр используется для указания допю информации не относящейся к структуре пути (фильтрация, сортировка, пагинация).
Параметр указывается в url после `?`, значение через `=`, `&` разделяет параметры.
`https://example.com/api/user/123/filter?page=1&size=10`
___
